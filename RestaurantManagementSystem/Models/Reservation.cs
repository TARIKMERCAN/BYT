...
namespace RestaurantManagementSystem.Models
{
    public class Reservation : SerializableObject<Reservation>, IDisposable
    {
        ...
        public void UpdateTable(Table newTable)
        {
            ...
            {
                ReservedTable.RemoveReservation(this); // Properly remove the reservation from the old table
            }

            // Use the IsTableReserved method from the Reservation class
            if (Reservation.IsTableReserved(newTable, DateOfReservation))
                throw new InvalidOperationException("The new table is already reserved for this date.");

            ReservedTable = newTable; // Update the reference to the new table
            newTable.AddReservation(this); // Add reservation to the new table
